  ...
  <div class="min-h-screen px-4 md:px-8 pb-16 max-w-6xl mx-auto">
    ...
    <div class="pt-8 pb-6 fade-up">
      ...
      <p class="text-text-secondary text-sm">
        模拟客户端向DNS服务器发起域名查询请求，展示DNS查询与响应的完整过程。
      </p>
    </div>
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="glass-card p-4 fade-up stagger-3">
          <div class="flex flex-wrap gap-2 items-center mb-4">
            ...
            <button class="btn-secondary text-sm" @click="handleNext" :disabled="isPlaying || currentStepIndex >= dnsSteps.length - 1">
              下一步
            </button>
          ...
          <!-- Step Indicator -->
          <StepIndicator :steps="dnsSteps" :current-index="currentStepIndex" />
        </div>
      </div>
        ...
        <div class="glass-card p-4 fade-up stagger-3">
          <div class="flex items-center gap-2 mb-3">
            <span
              ...
              :class="currentStep ? 'bg-accent-primary/20 text-accent-primary border border-accent-primary/40' : 'bg-white/5 text-text-muted border border-border-subtle'"
          ...
          <div v-if="currentStep" class="text-xs text-accent-primary font-medium">
            {{ currentStep.message }}
          </div>
        </div>
          ...
          <table class="table-glass">
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <span v-if="currentStep.direction === 'down'" class="text-accent-success text-xs">Response ↓</span>
              ...
              <tr>
                ...
                <td :class="currentStep.dstMAC === 'ff:ff:ff:ff:ff:ff' ? 'text-accent-warning' : ''">
                  {{ currentStep.dstMAC }}
                </td>
              </tr>
            </tbody>
          ...
          <table class="table-glass">
            ...
            <tbody>
              <tr v-if="dnsCache.length === 0">
                <td colspan="3" class="text-center text-text-muted py-3">暂无缓存数据</td>
              </tr>
...
const currentStep = computed(() => {
  ...
  return dnsSteps[currentStepIndex.value] || null
})

// Sync DNS cache with step data
const syncDnsCache = () => {
...
const autoPlay = () => {
  if (!isPlaying.value || currentStepIndex.value >= dnsSteps.length - 1) {
...
const handleNext = () => {
  isPlaying.value = false
  if (currentStepIndex.value < dnsSteps.length - 1) {
    currentStepIndex.value++
    syncDnsCache()
  }
}
